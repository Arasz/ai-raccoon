...
- **Corpus (default)**: the real-world corpus — 174 documents extracted from
  the user's actual repositories (job-search-ai-assistant, ai-badger,
  arasz-home-page: ADRs, invariants, skills, architecture docs) plus their
  `.remember` agent-memory notes, with 68 queries. Bodies are verbatim
  excerpts; relevance judgments are documented per query in the generated C#
  (`// judgment:` comments). Regenerated by
  `scripts/generate-benchmark-corpus.py` (read-only over the source repos).
