...
deterministic. Run `python3 .claude/hooks/verify_hooks.py`; non-zero exit means a defect.
"""
...
INCIDENT_8 = "pkill -f 'dotnet build'"
...
DENY = [
    ...
    "/usr/bin/pkill -9 dotnet",
    "sudo pkill dotnet",
    "sudo -u foo pkill -f 'dotnet build'",
    "sudo -u foo killall5",
    "sudo -- pkill dotnet",
    "env -i pkill dotnet",
    "sudo -n -u foo killall dotnet",
    "FOO=1 pkill dotnet",
    "ls && pkill dotnet",
    "echo hi\npkill -f dotnet",
    "kill $(pgrep -f dotnet)",
...
def check_lanes(marker_dir: Path) -> None:
    ...
    probe("incident-8 denied with 2 live lanes", _denied(INCIDENT_8, "s-lanes-2"))

    guard._live_lane_pids = lambda: [111]  # noqa: SLF001
    probe("incident-8 allowed with 1 live lane", not _denied(INCIDENT_8, "s-lanes-1"))

    guard._live_lane_pids = lambda: []  # noqa: SLF001
    probe("incident-8 allowed with 0 live lanes", not _denied(INCIDENT_8, "s-lanes-0"))

    guard._live_lane_pids = original  # noqa: SLF001
    guard.SOCK_DIR = marker_dir / "no-such-sock-dir"
    probe("incident-8 allowed when /tmp/cc-socks is absent",
          not _denied(INCIDENT_8, "s-nosocks"))
...
def check_valve(marker_dir: Path) -> None:
    ...
    outcomes = [_denied(INCIDENT_8, "s-valve") for _ in range(4)]
    ...
    probe("valve: incident-8 still denied after three harmless calls",
          _denied(INCIDENT_8, "s-harmless"))
    ...
    probe("valve: incident-8 still denied after three other hazards were denied",
          _denied(INCIDENT_8, "s-mixed"))
...
def check_pruning(marker_dir: Path) -> None:
    ...
    _denied(INCIDENT_8, "s-prune")
    probe("markers: a counter older than a day is pruned on write", not stale.exists())
    fresh = guard._denials_path("s-prune", INCIDENT_8)  # noqa: SLF001
...
def _edit(worktree: Path) -> Dict[str, Any]:
    return {"hook_event_name": "PreToolUse", "tool_name": "Edit",
            "tool_input": {"file_path": str(worktree / "src" / "Foo.cs")},
            "cwd": str(worktree)}
...
def check_dirty_cache_shape(cache_dir: Path) -> None:
    ...
    for label, content in (
            ...
            ("a non-string branch", '{"src/Foo.cs": [{"worktree": "x", "branch": 1}]}'),
            ("a non-list value", '{"src/Foo.cs": "injected text"}'),
