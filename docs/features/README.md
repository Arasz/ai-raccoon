...
| [`encryption-bitwarden/`](encryption-bitwarden/) | Implemented                             | [spec.json](encryption-bitwarden/spec.json) — encryption key sources: env (default, kept) + Bitwarden Secrets Manager via `bws` CLI (`encryption bitwarden`/`show`/`unset`/`migrate` CLI verbs) |
| [`code-corpus/`](code-corpus/)     | Agreed (implementation in progress, `task/code-mem-implementation`) | [spec.json](code-corpus/spec.json) — a second, code-only corpus (`code_entries`/`code_fts`/`vec_code`) searched via `memory_search kind=memory\|code\|both` + `code_get` |
