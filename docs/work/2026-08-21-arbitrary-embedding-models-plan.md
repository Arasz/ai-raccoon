...
**Date:** 2026-08-21
**Task:** support-for-other-embedding-models (plan-only; no implementation in this task)
...
| D6 | Chunk budget for manifest-local models = `ctx − 2` capped at a NAMED constant `MaxManifestChunkTokens = 510` ⟲ (F14), derived from the tokenizer's `SpecialTokenReservation`; **`TrimQueryToWindow` also trims to 510 for manifest models** ⟲ (F10c); remote stays 8191. The budget change lands in WP3, confined to manifest models ⟲ (F8). SentencePiece bos/eos double-reservation must be pinned by parity fixtures (CountTokens vs ids-minus-specials semantics) ⟲ (m4). |
...
## 8. Still open for the owner (in Rider)

1. Approve the rev-2 plan (G0) — then WP1-WP4 can start (still no behavior change before WP5).
2. Decide whether WP5's bge-m3 eval report may CHANGE the shipped default model (separate ADR; the plan itself never changes defaults).
3. Decide whether `model download` ships in the same PR as the engine generalization or as a follow-up (it is optional for the core ask).
