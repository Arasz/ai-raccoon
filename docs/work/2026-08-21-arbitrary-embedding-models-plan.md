...
| # | Decision |
|---|---|
| D1 | **Manifest contract — ONE pinned JSON schema** ⟲ (review B2/F1): sidecar **`ai-raccoon.manifest.json`** (distinct name — model repos may ship their own `manifest.json`; e.g. faxenoff/code-daemon-embed-v1, found by the #405 pre-implementation audit) next to local model files; canonical shape = nested (architecture §5.6) extended with `requiresTokenTypeIds` and a **numeric special-token map** taken from `special_tokens_map.json`/`tokenizer_config.json` at download time (no `<mask>` mapping — xlm-roberta mask id is 250001, model-specific); `pooling.mode ∈ {mean, cls, model-output, last-token}` (engineer's C# enum MUST include `model-output`); bundled model = compiled-in manifest; remote = settings rows (+ `embedding.dimensions`); legacy `embedding.model=<path>.onnx` keeps working. WP1 golden fixtures + WP2 mocked-downloader fixtures both pin this schema. |
...
| D6 | Chunk budget for manifest-local models = **`min(MaxManifestChunkTokens, ctx − specialTokenReservation)`** with `MaxManifestChunkTokens = 510` a NAMED constant ⟲ (review F14; amended for the #405 audit): 510 is the CEILING for large-window models (510 = 512 − 2: one 512-token window minus the reservation; bge-m3's MCLS pooling applies ABOVE 512 and is deferred until parity work, so CLS-pooling semantics are only valid within 512). The model's own window bounds it below — a 128-ctx model resolves to 126 because the ONNX graph does NOT truncate and over-length chunks would be silently broken. `TrimQueryToWindow` trims to the same effective budget for manifest models ⟲ (F10c); remote stays 8191; bundled stays 254. The budget change lands in WP3, confined to manifest models ⟲ (F8). SentencePiece bos/eos double-reservation must be pinned by parity fixtures (CountTokens vs ids-minus-specials semantics) ⟲ (m4). |
