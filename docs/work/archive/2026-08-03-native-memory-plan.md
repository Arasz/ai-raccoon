...
| C | **P6** hybrid search RRF (FR-NM-4) incl. **SearchResultMerger RRF rework**; **P6b** ceaksan ADOPT refinements (candidate window K=max(limit*3,100) per modality, distance-ascending discipline, snippet fallback for vector-only hits, chunk default 512→256/overlay 32-48 tied to model context) — then re-run the harness | P6b serial after P6 | 4 search scenarios; harness parity nDCG Δ≤0.02 vs vendored reference (GGUF) on the shared corpus; P6b re-verified parity unchanged |
...
## 8. Research addendum — ceaksan sources (e: 2026-08-03)

Sources: hybrid-search-fts5-vector-rrf (fetched full, MEASURED); rag-chunking-guide (PAYWALLED — only public TL;DR/FAQ analyzed; body UNVERIFIED, no guesses).

- **ADOPT (P6b, Wave C):** (1) candidate window K = max(limit*3, 100) per modality before RRF fusion — per-modality LIMIT @limit starves overlap candidates; (2) vec_distance_cosine is a DISTANCE (0=identical, up to ~2) — rank ascending, never score with it; (3) snippet fallback for vector-only hits (no FTS MATCH → no snippet()) — trim value ~200 chars + '…' keyed by hash, keeps FR-NM-4 snippet-on-every-result; (4) chunk defaults 512 tokens exceed all-MiniLM-L6-v2's 256-token window → truncation dilution; default maxTokens 256 (or 128–192 per S1's ~340-char sweet spot), overlay 32–48, chunk size tied to configured model context. FR-NM-10 pins bounds, not sizes — contract-safe.
- **CONSIDER (Wave F / follow-up, each with own TDD + harness validation):** prefix-retry fallback tier (zero FTS5 hits → term-prefix expansion); heading-aware splitting (## / ### boundaries, ~200-char minimum merge, deterministic); chunk-size × overlay sweep as a harness axis; contextual chunk headers (heading-path/file-path prepended to EMBEDDING INPUT only, one-time re-embed, harness-validated); e5-family query:/passage: prefix rule documented when a remote e5 model is configured.
- **REJECT:** semantic (embedding-similarity) chunking (violates FR-NM-10 determinism, 5–10× slower); TF-IDF routing layer (ai-badger's domain); tree-sitter/AST symbol indexing (new feature + parser dep; future idea only); ColPali/vision PDF chunking (no PDF ingestion in contract); Chonkie/LlamaIndex/RAGAS (Python/deps, no value over our C# harness); late chunking (model context too small; contextual headers capture most of the gain); non-default RRF k/weights without harness sweep evidence.
