...
delegates with the first discriminator as fallback; header gains
`topicDerivation`. Corpus `project-corpus-100.json` regenerated by the
generator: sha `0a7ecb013364…` → `5445425c695a…`, debris **23/100 → 0/100**
(independent `evaluate.debris_query`; repair is target-preserving — every
`id ↔ anchor/scope/project` mapping, holdout and allocation unchanged).
Re-run artifacts: `docs/work/results-f2.json` + `docs/work/results-f2-run2.json`
(pair diff CLEAN), report `docs/work/2026-09-10-corpus-quality-eval-report.md`
(strata debris n=0 / clean n=99), method
`docs/work/2026-09-10-corpus-quality-clean-instrument-lane-report.md`.
Delta: aggregates harness 0.677/0.158 → 0.828/0.192, bank 0.808/0.187 →
0.879/0.203, MCC 0.5955 → 0.3233, cont [65,2,15,17] → [76,6,11,6], stale [C035]
both; clean subset old n=76 @ 0.697/0.842 → new n=99 @ 0.828/0.879
(composition+text delta, not a controlled A/B — f1 stays the frozen reference
for the old corpus only). f1 files byte-identical; `eval-set-100.json`
byte-identical (`5cc5bcdf…`).
