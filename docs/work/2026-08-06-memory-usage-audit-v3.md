...
- **First auto-promote pass not yet observed — and impossible while no running server has the #55 binary** (all stdio servers predate the 14:42-local swap; the post-swap instance exited quickly). Verify after the next Hermes MCP session or a 5094-bridge restart on 1.0.9: it should fire ~60 min after start, dedup against the 3 existing shared rows, and avoid the S1 interval-read exception path.
