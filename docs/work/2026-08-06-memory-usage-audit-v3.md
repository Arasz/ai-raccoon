...
**B2 — Extract auto-promotion is CONFIGURED and RUNNING (correcting the interim reading): enabled=True, mode=promote, 60-min interval; first auto-pass expected ~13:45 UTC, after this window [MEASURED]**
`ai-raccoon extract list` on the installed tool: `enabled: True, mode: promote, interval: 60 min`. The 1.0.9 stdio servers (banners from 12:39 UTC) load the #55 binary, so the hosted service is live in promote mode. Caveats (from the #55 review): the interval read sits outside the exception shield (a config-read failure can StopHost-kill the server, SHOULD-FIX S1), and promote shares without confirmation by design. The old bridge cannot run the service; the adoption plan's "bridge ≥1.0.8" precondition remains unmet for it.
...
- **First auto-promote pass not yet observed** (expected ~13:45 UTC = 15:45 local, after the window). Verify it fires, dedups against the 3 existing shared rows, and doesn't hit the S1 interval-read exception path.
