...
| **Code chunk size vs recall** (new, from #422/PR #453) | scheduled | next retrieval-tuning round, with M5 | #422's measurement settled what the code-daemon graph *permits* (512 tokens), not what retrieves best. `CodeChunker.DefaultBudget` moved 126 → 510 accordingly, but the model card recommends short units and its training regime is short code spans, so 4x-larger chunks are an untested recall trade. Rows ingested before #453 keep their 126-token chunks until each file is re-ingested — deliberately no re-ingest trigger (ADR-0087's invalidation is for *correctness*; a smaller chunk under a wider budget is only quality, and code is re-derivable). Also needs the same env-var wiring as the parity gate above: `CodeModelGraphWindowTests` is Nightly-tagged and gated on `AIRACCOON_CODE_MODEL_DIR`, which nothing sets. |
