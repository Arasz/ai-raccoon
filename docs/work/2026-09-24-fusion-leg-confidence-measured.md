...
> **Not merged.** The implementation this record measures (PR #730, branch `task/air-fusion-confidence-weighted-rrf-706`, head `59352940`) was closed unmerged: the flag failed its keep rule and would have shipped off. `LegConfidence`, `FusionConfigKeys.LegConfidence*` and `LegConfidenceFusionGateTests` exist only on that branch. The findings stand as a measurement of the approach.
