...
worked around by copying first, matching the harness's own sanctioned
read-only-live/writable-copy split). Vectors read via the exact native binary
...
at line 49) — 512 tokens total once the two special tokens are added. Since
...
- The live bank has grown substantially since the corpus was pinned (the
  corpus's `snapshotSha256` predates today's copy by design — a fresh copy
  always warns on this, per `ingest.py`'s documented behavior — and is not
  itself part of this task's scope).
