...
> **Rev-1 MoE snapshot — SUPERSEDED WHERE IT CONFLICTS.** This document is the architect lane's
> historical MoE record. Where it and the combined plan's §3 decisions disagree, the combined
> plan wins — notably **D3** (dimension reconcile: single-transaction DROP+CREATE, no repopulate,
> no every-open-DDL heal, DDL stays `float[384]`) and **D6** (chunk budget: `MaxManifestChunkTokens
...
3. **Dimension reconcile** (new, idempotent, runs inside the migration lock so no tool call can interleave — ADR-0076 ToolGate): read the actual vec0 dimension from `sqlite_master` (`SELECT sql FROM sqlite_master WHERE name IN ('vec_entries','vec_structure')`); if it differs from the target → `DROP TABLE vec_entries; DROP TABLE vec_structure;` — the unconditional DDL (`CREATE … IF NOT EXISTS` with `float[N]`, F10) recreates them (the DDL's dimension becomes a parameter of the bank's active engine rather than a constant). **> SUPERSEDED by rev-2 D3:** the DDL is digest-gated (not every-open), and the parameterized-DDL idea is dropped; the reconcile is a single `BEGIN IMMEDIATE` transaction doing DROP+CREATE at `float[N]`, with table PRESENCE read explicitly (the 384 fallback conflates missing with 384).
...
5. Failure safety: a crash between DROP and CREATE is healed by the every-open DDL (F10); a crash mid-drain leaves the outbox open and the relay retries (F2/F11 machinery). **> SUPERSEDED by rev-2 D3:** there is NO every-open DDL (digest-gated); the single-transaction reconcile makes a crash between DROP and CREATE roll back cleanly, and G4(b) tests that exact window.
...
| WP4 | eng | Dimension reconcile (D3): vec0 DROP+CREATE with `float[N]` inside the migration relay; `embedding.dimensions` settings row (remote); `--dims` on `model set openai`; pre-WP5 gate: `model set local` refuses manifest dims ≠ 384 | **> SUPERSEDED by plan §5 G4** (this row is the weaker rev-1 gate; the plan's G4 adds vec_structure parity, kill-9 between DROP and CREATE, wrong-`--dims` refusal with outbox NOT committed, undeclared-dims fail-closed, and the reconcile-disabled negative). Original row: G4: fixture-bank tests on a scratch server — (a) 1024-dim manifest + tiny committed fixture ONNX (test-only, ~MBs) migrates: counts parity (entries == vec rows), `sqlite_master` DDL now `float[1024]`; (b) crash mid-migration (kill -9 the relay) recovers on restart (outbox still open → retry); (c) legacy 384 bank untouched (DDL unchanged); (d) `model set openai --dims 1024` writes the row and the fingerprint includes dims |
