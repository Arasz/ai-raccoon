...
| R15 | Protocol switch (owner f:) — WebSocket-style stdio→http banner | DEFERRED: no standard exists (spec transports page); client side needs a ~110-line fork of the SDK `stdio_client` (JSON-parses every stdout line, no pre-spawned process); attach + url-config covers the need; zero-arg auto-banner rejected (breaks non-peeking clients). This plan document is the design record. **Superseded by [ADR-0020](../adr/0020-always-on-http-stdio-proxy.md):** the always-on stdio-to-HTTP proxy makes a protocol-switch banner unnecessary — the stdio front end relays to the HTTP backend transparently instead of ever renegotiating transport with the client |
| R16 | ai-badger `.mcp.json` → `ai-raccoon serve` is TRANSPORT-BROKEN (scaffolder emits stdio-shaped entries; serve never answers JSON-RPC; Claude Code never reads the printed URL; attached mode exits before initialize) | `.mcp.json` stays stdio. ai-badger PR scope: Hermes url: guidance (server.md) + meta.json fixes (`package: "ai-raccoon"`, description) — see §5. **Superseded by [ADR-0020](../adr/0020-always-on-http-stdio-proxy.md):** `.mcp.json` and the ai-badger scaffolder still emit a bare stdio-shaped `command` entry, and that is now sufficient — it launches the proxy, which does answer JSON-RPC by relaying to an autostarted `serve` backend. The TRANSPORT-BROKEN problem this ruling worked around no longer exists |
