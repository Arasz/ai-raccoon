...
|---|---|---|---|
| 1 | `serve --port 7799 --mcp-entry --format all` on a scratch data root | both documents carry `"headers":{"X-AiRaccoon-Token":"${AIRACCOON_MCP_TOKEN}"}`; token file written 0600, 43 bytes; no live secret in either document | pass |
| 2a | curl `X-AiRaccoon-Token: <token>` | 200, `serverInfo AiRaccoon` | pass |
| 2b | curl `Authorization: Bearer <token>` | 200 | pass |
| 2c | curl `Authorization: bearer <token>` (lowercase scheme) | 200 | pass |
| 2d | curl, no credential | 401, "needs the X-AiRaccoon-Token header or Authorization: Bearer; the token is in `<path>`" | pass |
| 2e | curl `Authorization: Basic <token>` | 401, **mismatch** message | pass |
| 2f | curl `X-AiRaccoon-Token: ${AIRACCOON_MCP_TOKEN}` (unexpanded placeholder — the D4 failure mode) | 401, "got a X-AiRaccoon-Token or Authorization value that does not match the token in `<path>`" | pass — this is R4's whole purpose |
| 2g | curl wrong custom header **plus** correct Bearer (R8) | 200 — neither envelope shadows the other | pass |
| 5a | plugin `transport: http` + `token_file` against the gated server | `memory_stats` returned `{'entries': 0, 'pending': 0, 'contexts': []}`; `_http_client.is_closed` true after `close()` | pass — the regression, fixed |
| 5b | same, token file missing | before: `CancelledError`, empty message. after: `AiRaccoonError: ai-raccoon connect to http://127.0.0.1:7799/mcp (X-AiRaccoon-Token not sent; token file <path>) failed: CancelledError` | fixed during QA |
| 5c | plugin pointed at a **remote** host with a token file present (R3) | no auth header attached | pass |
| 5d | plugin with a **wrong** token (the wrong-bank case) | `AiRaccoonError … (X-AiRaccoon-Token sent; token file <path>)` | pass |
| 6 | bare `ai-raccoon --data-root <fresh> --port 7801` (proxy default) | backend started, token minted 0600, `initialize` round-tripped; backend confirmed gated (401 without a credential) | pass — the path that was never broken stays working |

Routes 3 and 4 (`hermes mcp add` / `claude mcp add`) were **not** run here: both write to the
operator's live client configuration, and this task ran unattended. What they depend on was
measured instead — Hermes's recursive `${VAR}` interpolation (`mcp_config.py:254-276`) and Claude
Code's expansion of `${VAR}` inside `headers`, including the unset-variable behaviour that sends
the literal placeholder, which row 2f proves the server now diagnoses. The routes themselves
remain untested end to end; that is the residual this table does not close.

A note on harness error, recorded because it briefly looked like a gate bypass: a first curl sweep
returned 400 for every case including the uncredentialed one. The cause was zsh, which does not
word-split unquoted expansions, so a `$H` variable holding two `-H` flags reached curl as a single
malformed header and Kestrel rejected it before any middleware ran. Written properly the same
requests give the 401/200 above.
