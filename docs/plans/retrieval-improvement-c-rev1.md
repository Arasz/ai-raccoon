...
3. **AND with OR fallback**: join remaining tokens with AND when ≤4 tokens; if the MATCH returns
   zero rows, retry with OR-join. This captures AND's precision benefit for short queries while
   preventing the zero-match regression measured on old corpus (A2, A6).
...
Diagnostic triplet confirms FTS-only path answers A7 on the clean corpus (target: rank ≤5
FTS-only — the hybrid already delivers it at rank 1). The AND-fallback prevents any zero-match.
...
content). A query for a source path (e.g. `docs/adr/0011-...#decision`) returns that exact chunk.
No regression on invariants (C1/C2/C5 at rank 1).
...
4. **Parameter sweep** over λ, consolidation threshold, and document-score formula against
   the full baseline.
5. **BM25 length normalization** investigation: the old corpus showed length was a contributor
   but not the dominant cause of A7's ranking gap (raw-TF moved rank 11 → 6, still outside
   top-5). On the clean corpus, re-measure.
...
3. Add structural and cross-document test cases (S1-S6 from Appendix A)
4. Compute nDCG@5, recall@5, MRR for all queries including structural cases
5. Permanent corpus-integrity assertions in the harness

**Gate**: Baseline report includes ablation breakdown, retrieval metrics, difficulty
stratification, and corpus integrity checks. H1-H3 negative tests pass because the corpus
excludes the content (not by luck). Structural queries S1-S6 scored.
...
> re-measure on clean corpus before shipping).
...
4. **Gate**: section-targeted queries (S2 "What does ADR-0011 decide?", S4 "Consequences of
   ADR-0011?") at rank ≤3. Section-level hit@5 over A1–A7 improves over content-only baseline on
   the clean corpus. No regression on content-only file-level ranks.
...
| Section-targeted hit@5 | Not measured | Not measured | ≥4/6 (Wave 6) |
| Dual-vector MRR(section) | — | — | ≥0.40 (Wave 6) |
...
Wave 0 (DONE ✓ — reproducible baseline)
 ...
 └── Wave 5 (baseline enrichment) ── depends on 3+4+6 (measures final state including
      structural queries + section-targeting)
```
