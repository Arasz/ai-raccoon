...
| [0012 — SSH-key derivation → HKDF](0012-ssh-key-derivation-hkdf-replacement.md) | Replaces the hand-rolled `SHA-256(label ‖ seed)` bank-key composition with platform `HKDF-SHA-256`; existing Bitwarden/SSH-keyed banks need a `PRAGMA rekey`; implementation is a separate work item
