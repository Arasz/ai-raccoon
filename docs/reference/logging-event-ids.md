...
Measured directly against `src/` on this branch: **162** `[LoggerMessage]`-attributed
...
| 418-419 | `src/AiRaccoon.Infrastructure/Embedding/EmbeddingService.cs` (418 added 2026-08-15 as **416**: a search query trimmed to the model window, ADR-0071 — split out of 414 so each is countable. Moved 416 -> 418 on 2026-08-22, #466, so `OnnxEmbeddingGenerator` could extend to 417 without interleaving; ADR-0071 and ADR-0072 name the old number and carry an amendment. 416 is retired. 419 added 2026-08-22, WP11-A/G16: `embedding.threads` didn't parse to a non-negative integer, so the halved-core default is used instead — the block's next free id, no longer left open for `ManifestPoolingRepair`. This owner is now itself wedged the same way `OnnxEmbeddingGenerator` was: 420-425 (`NoOpCodeChunker`/`CodeEmbedder`/`ManifestPoolingRepair`) sit immediately above it, so #522's new session-created line could not extend this block without interleaving — see 426) |
...
| 426 | `src/AiRaccoon.Infrastructure/Embedding/EmbeddingService.cs`'s `EmbeddingSessionLog` — a distinct top-level type, not nested in `EmbeddingService` (added 2026-08-23, #522: the resolved ORT intra-op thread count a local session was actually built with — the round-tripped `embedding.threads` setting had no observable confirmation it took effect. Both natural homes, `OnnxEmbeddingGenerator` [414-417] and `EmbeddingService` [418-419], were wedged with `NoOpCodeChunker`/`CodeEmbedder`/`ManifestPoolingRepair` occupying 420-425 immediately above `EmbeddingService`'s block — extending either owner's range to 426 would engulf those three owners' ranges and fail `EventIdBlocks_DoNotInterleaveBetweenOwners`. A single-id owner right after that cluster needed no renumbering) |
