...
Measured directly against `src/` on this branch: **155** `[LoggerMessage]`-attributed
...
| 418 | `src/AiRaccoon.Infrastructure/Embedding/EmbeddingService.cs` (added 2026-08-15 as **416**: a search query trimmed to the model window, ADR-0071 — split out of 414 so each is countable. Moved 416 -> 418 on 2026-08-22, #466, so `OnnxEmbeddingGenerator` could extend to 417 without interleaving; ADR-0071 and ADR-0072 name the old number and carry an amendment. 416 is retired) |
...
| 424-425 | `src/AiRaccoon.Infrastructure/Embedding/ManifestPoolingRepair.cs` (added 2026-08-22, #470: the manifest correction that retires a standing 417 — 424 records the pooling block rewritten to 'model-output' at engine activation, 425 warns when it could not be written and 417 will therefore keep firing. 419 stays free: this owner needed two adjacent ids and `NoOpCodeChunker` holds 420) |
