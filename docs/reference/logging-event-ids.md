...
Measured directly against `src/` on this branch: **182** `[LoggerMessage]`-attributed
...
(Remeasured 2026-09-22, task/psr-w3-p31: **182** — `DoctorCommands` 1016, the
bank-is-not-a-database warning behind exit 26 (F7). The whole block relocated to
1014-1016: the new id could not grow in place because 1002 was already
`EmbedDrainReporter`'s, the same wedge as 416/418/424.)
