...
| `sync-alias-conflict` | The pull found a `project_id_aliases` row whose alias the local bank already maps to a different winner (or kind) — first-writer-wins keeps the local row and the merge aborts before entries move, naming the alias and both winners for a human to reconcile | `sync-alias-conflict: <detail>` |
