...
| `sync-tampered-remote` | The pulled remote snapshot's HMAC authenticity tag does not match its bytes (checked before `PRAGMA quick_check` and before `ATTACH`) — the local DB is not replaced. An encrypted bank keys the tag from its own passphrase via `HKDF`; a legacy remote with no tag is accepted with a logged warning instead of refused | `sync-tampered-remote: <detail>` |
