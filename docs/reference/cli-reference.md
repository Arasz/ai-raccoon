...
| `settings model device {auto\|gpu\|cpu}` | Where local embedding sessions run: `auto` (default) puts only the bundled model on the GPU; takes effect on next restart (ADR-0108) |
