...
2. **Compare with the sibling hosted service in the same repo** — same-repo precedent is the strongest review signal. In the project, the sibling hosted service try/catches both the reconcile AND the delay; a new service that guards less is a finding. Cite the divergence explicitly.
...
7. **Logging invariant** — nested static partial `Log`, `[LoggerMessage]`, explicit EventIds. Check collisions against the repo's per-class ranges (e.g. 1-5, 100, 200-205, 300-330, 400, 500-505). EventIds are scoped per logger category, so cross-class reuse is benign — note only same-category collisions.
...
A worked review record (PR case study: finding shapes with evidence lines and the
