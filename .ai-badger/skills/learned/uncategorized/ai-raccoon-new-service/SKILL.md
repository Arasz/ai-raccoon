...
- **McpToolContractTests type-mapping gotchas.** The `ExpectedContract` raw string
  literal uses MCP JSON schema types, NOT C# types:
  - C# `int` → `integer` (not `int`)
  - C# `string?` (nullable reference) → `string|null?` (not `string?`)
  - C# `int?` → `integer|null?`
  - Raw string literal indentation must match the closing `"""` exactly (CS8999 otherwise).
  Run `dotnet test --filter "InputSchemas_MatchTheDeclaredContract"` after any tool addition.
- **Subagents can accidentally delete unrelated files.** Always check `git status --short`
  after parallel subagents finish and restore unexpected deletions with
  `git checkout HEAD -- <path>` before committing.
