...
- Missing `<PackageVersion>`: .sh fails late with `FAIL: …/ai-raccoon..nupkg was not produced` (empty version in filename); Python fails early with `FAIL: <PackageVersion> not found in <csproj>` — same exit 1, clearer.
