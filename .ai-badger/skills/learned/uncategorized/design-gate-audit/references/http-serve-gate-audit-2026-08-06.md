...
Design: `docs/work/2026-08-06-http-serve-design.md` (ai-raccoon worktree `switch-from-stdio-to-http`).
Report: `docs/work/2026-08-06-http-serve-moe-test-engineer.md`.
...
| Routing/dispatch gates | A13 "no serve path leaks into ConfigCommands" — the negative pin passes both with and without the Program.cs `["serve"]` branch; needed an `EntryPointRouter` seam + positive pin. |
| Unpinned log claim | A12 "bare launch still logs HttpTransportListening" — no test pins that log (E2E factories run at Warning minimum; only `McpServerSetup.Log` is LoggerMessage-pinned). |
...
| Env poisoning | ServeRunnerTests resolve the encryption key; `AIRACCOON_DB_PASSPHRASE` on a dev machine flips exit 0 → 2 — use `TestData.EnvVarGate`. |
...
| Claim verification | "no test churn" — grep `new ServerConfig(` = 0 literals in tests; exactly 1 target-typed 3-arg `new(...)` (McpServerSetupHostTests.Config) + 1 src call site; defaulted 4th positional keeps both compiling → claim HOLDS. |

## Verified-existing test infrastructure (feasibility anchors)

- Host-shape pins: `McpServerSetupHostTests` `GetServices<IHostedService>().ShouldContain/ShouldNotContain(service is ExtractionHostedService)`.
