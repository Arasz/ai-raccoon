...
Symptom (user f:): "other projects don't see ai-raccoon memory mcp". All facts
...
1. **Tool command names change between versions.** arasz.ai-raccoon 1.0.4
   installed the binary as `AiRaccoon` (capital — evidence: a hand-written
   VS Code entry referencing `~/.dotnet/tools/AiRaccoon`); 1.0.5's
   ToolCommandName is `ai-raccoon` (lowercase shim). Every scaffold between
...
1. Upgrade the tool so its command matches the catalog:
   ...
   additions — same class of issue as the hermes/ai-raccoon conditional).
...
3. Verify: the project's `.mcp.json` carries the entry. Verified behavior: a
   ...
   job-search-ai-assistant: AIRACCOON_DATA_ROOT / AIRACCOON_RID lost). After a
...
Hermes sessions (observed 2026-08-05: "other projects don't see ai-raccoon" was
...
## ai-raccoon: single-instance port-5000 bind + the stdio probe

The server binds `http://127.0.0.1:5000` (Kestrel; `WebApplication.CreateBuilder`
in `src/AiRaccoon/Program.cs`) on EVERY launch — stdio included. A second instance
...
ai-raccoon (a Hermes watchdog instance, a `--transport http` server, a stale dev
build) holds the port. macOS ControlCenter's `*:5000` coexists (SO_REUSEPORT) and
is NOT the blocker; only a second ai-raccoon on 127.0.0.1:5000 is. Only one
ai-raccoon instance can run per machine.
