...
Verified 2026-08-06 reviewing ai-raccoon PR #65 (server `--quiet`: info logs → Warning+).
...
Concrete case: `ai-raccoon --transport http --port 0 --quiet`. `--port 0` = bind a random free port; the ONLY
...
from `McpServerSetup.Log` to `HostExtensions.Log` during a refactor), the old definition stays behind — same message,
