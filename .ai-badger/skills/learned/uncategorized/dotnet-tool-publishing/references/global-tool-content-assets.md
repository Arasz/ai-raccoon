# Global-tool content assets: the ai-raccoon 1.0.4 case (measured 2026-08-05)

Full worked case behind the "Bundled content assets" section of SKILL.md.

## Incident

The installed tool (arasz.ai-raccoon 1.0.4, macOS arm64) ran but `memory_search` failed:
...
1. **Settings value resolved as a cwd-relative path.** The bank's settings table held
   ...
   message names the resolved path and both remediations — `'ai-raccoon model set local'`
   (bundled) or `'ai-raccoon model set local <path-to-onnx>'` (custom). The settings row
...
```
~/.dotnet/tools/.store/arasz.ai-raccoon/1.0.4/
├── arasz.ai-raccoon/1.0.4/                      # shell package (root also carries Models/)
│   └── Models/vocab.txt
└── arasz.ai-raccoon.osx-arm64/1.0.4/
    ├── Models/vocab.txt                          # payload root copy
    └── tools/net10.0/osx-arm64/
        ├── AiRaccoon.dll ...                     # AppContext.BaseDirectory
...
- **`lsof -p <pid> -iTCP -sTCP:LISTEN`** on the running tool found the port (5094) AND the
  exact store paths (`txt REG` lines point at `.../osx-arm64/AiRaccoon.dll` etc.) AND the
...
- `shasum -a 256` of the copied file == the constant pinned in `BundledModel.cs`
  ...
  `src/AiRaccoon/Models/` is valid by construction.
...
- Resolver fail-fast + message contract; stale env-var guidance
  (`AIRACCOON_EMBEDDING_MODEL`, `--embedding-model` — both deleted by the single-channel
