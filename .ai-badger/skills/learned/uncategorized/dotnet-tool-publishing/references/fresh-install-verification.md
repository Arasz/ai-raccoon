...
Measured 2026-08-06 on `arasz.ai-raccoon` 1.0.6 (osx-arm64), executed as
`scripts/manual-fresh-install-test.py` (committed in the ai-raccoon repo). The protocol is
...
- `dotnet tool install --tool-path $TOOLPATH --version <v> arasz.ai-raccoon` — a temp tool
...
- `--data-root $DATAROOT` (fresh dir) — bypasses the tool's default `~/.ai-raccoon` so the
  test never reads or writes real user data.
- `unset AIRACCOON_DB_PASSPHRASE` — an inherited secret env var silently flips the tested
...
1. Run the documented setup verb as part of the happy path: `ai-raccoon --data-root
...
- `serverInfo.name` = assembly name (`AiRaccoon`), `serverInfo.version` = AssemblyVersion
...
as an env override (`AI_RACCOON_VERSION`, default = current) so the gate re-runs against
