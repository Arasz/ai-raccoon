# NuGet package id migration — worked example (ai-raccoon `arasz.ai-raccoon` -> `ai-raccoon`, 2026-08-06)

Task record in-repo: `docs/work/2026-08-06-nuget-package-id-migration.md`. This file
...
Both packages install `ToolCommandName=ai-raccoon`, so users could never hold both
...
6. READMEs: install line + a migration note (uninstall old THEN install new;
   data root is keyed to install scope, not package id — `~/.ai-raccoon`
