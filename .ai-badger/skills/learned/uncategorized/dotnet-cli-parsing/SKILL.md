...
ai-raccoon MCP server — `docs/work/2026-08-04-cli-args-exploration.md`,
`docs/plans/cli-args-parsing.md` in the ai-raccoon repo.
...
## Maintaining an existing Cocona tool (verified 2026-08-05, dotnet-ignore)
...
`references/system-commandline-2.0.10-ga-api.md`; instance-based reads + custom
value-grammar validation via `Option.Validators` (verified 2026-08-06):
`references/system-commandline-2.0.10-option-validation.md`. The short version:
...
is the only runtime channel), the pattern that worked (ai-raccoon, 19-command verb
tree):
...
- **Dispatch shape:** `Program.cs` parses args; if the parse produced a command
  ...
  `ConfigCommands.RunAsync(commandPath, parseResult, store, stdout, stderr, Console.In)` and
  return the exit code — **never start the MCP server**. Bare `ai-raccoon [flags]`
...
- **One-shot CLIs cannot see runtime state — name read commands by what they CAN
  show (verified 2026-08-05, ai-raccoon `watch registered`):** a config verb runs
...
- Owner rulings can override this (2026-08-04 ai-raccoon: secrets moved into the settings
  ...
  line, trims, and on empty input writes `ai-raccoon: <name> key required — …` to stderr,
...
- `.mcp.json`: `{"mcpServers":{"<name>":{"command":"<tool-name>","args":[...],"env":{...}}}}` —
  dotnet tools appear as bare command names (e.g. `"command": "ai-raccoon"`), unlike
