# Serve mode: client-config contract + watchdog DI triple (2026-08-06, ai-raccoon)

Verified while shipping the HTTP serve mode (design `docs/work/2026-08-06-http-serve-design.md`,
branch `task/switch-from-stdio-to-http`). The probe-attach HTTP facts live in
...
`config.yaml` `mcpServers` (`{"ai-raccoon": {"url": "http://127.0.0.1:7721/mcp"}}`)
