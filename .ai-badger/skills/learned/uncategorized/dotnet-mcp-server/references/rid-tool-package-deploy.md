...
Verified 2026-08 while reviewing an MCP-server task (AiRaccon, `dotnet pack` deploy target).
...
| `Id.<rid>.Version.nupkg` | hundreds of KB (payload) | `tools/net10.0/<rid>/<exe>` + `AiRaccon.deps.json`, `runtimeconfig.json`, all dependency DLLs |

The shell's `DotnetToolSettings.xml` redirects the installer to the companion:

```xml
<DotNetCliTool Version="2">
  <Commands><Command Name="AiRaccon" /></Commands>
