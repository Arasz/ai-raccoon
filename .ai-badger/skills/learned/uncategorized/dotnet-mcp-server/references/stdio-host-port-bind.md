...
Verified 2026-08-05 against ai-raccoon 1.0.5 (ModelContextProtocol SDK 2.0, `WebApplication`
host, dual transport). First fixed in `fix/stdio-port-bind` (ai-raccoon PR #28, ephemeral
bind); superseded by the plain-host refactor below (task ai-raccoon-mcp-setup-refactor).
...
`*:5000` but coexists (SO_REUSEPORT); two ai-raccoon instances do NOT coexist.
...
the ai-raccoon setup refactor) is: **stdio-only servers don't need a `WebApplication` at
