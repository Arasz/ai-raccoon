...
Context: ai-raccoon CLI-args wave (2026-08-04), System.CommandLine 2.0.10 (GA; net8.0 +
...
- **Command name defaults to the ASSEMBLY name, not PackageId.** Package `ai-raccoon` installed
  a shim named `AiRaccoon`:
  `~/.dotnet/tools/AiRaccoon → .store/ai-raccoon/0.1.0-beta/ai-raccoon.osx-arm64/0.1.0-beta/tools/net10.0/osx-arm64/AiRaccoon`.
  Works on case-insensitive filesystems (Windows, default APFS) by luck; breaks `.mcp.json`
  `"command"` on Linux/case-sensitive macOS. Fix: `<ToolCommandName>ai-raccoon</ToolCommandName>`.
  **Fastest verification — before installing anything:** `dotnet pack` once (host RID), then
  ...
  `unzip -p <out>/ai-raccoon.0.1.0-beta.nupkg tools/net10.0/any/DotnetToolSettings.xml`
  → must contain `<Command Name="ai-raccoon" />` (and no `<Command Name="AiRaccoon"`). This
...
```bash
{ printf '%s\n' '<frame>'; sleep 3; } | ai-raccoon 2>/tmp/ai-raccoon-smoke.err | head -c 400
```
