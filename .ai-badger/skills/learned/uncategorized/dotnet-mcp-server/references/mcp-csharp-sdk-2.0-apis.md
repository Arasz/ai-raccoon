...
## Applied example — AiRaccon "agent memory server" Wave 3 (repo state snapshot)

Repo: `~/RiderProjects/ai-raccon/AiRaccon`, worktree `.ai-badger/worktrees/agent-memory-server`,
branch `task/agent-memory-server`. `TreatWarningsAsErrors`, `Directory.Packages.props` central
versions, `InternalsVisibleTo AiRaccon.Tests` per project. Store integration tests are NOT run
...
- **Port-gap analysis** (spec `docs/features/agent-memory/spec-issue-1.md` §4.1, 17 tools):
  ...
  **it is not** (`Directory.Packages.props` has no NSubstitute); existing tests use hand-written
  fakes (see `tests/AiRaccon.Tests/Domain/MemoryStorePortTests.cs` `RecordingStore`).
