...
How to add a new pure-domain class library to an existing .NET solution, TDD-first, without touching other layers. Validated in the AiRaccon agent-memory worktree (`src/AiRaccon.Core`, net10.0, xunit.v3 + Shouldly, TreatWarningsAsErrors repo).
...
2. `dotnet test` → every file fails with the same `CS0234: type or namespace 'AiRaccon.Core' does not exist` — that is legitimate red; no stubs needed (nothing is masked, the error IS "feature missing").
