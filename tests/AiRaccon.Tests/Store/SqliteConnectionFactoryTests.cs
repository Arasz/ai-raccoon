...
public sealed class SqliteConnectionFactoryTests : IDisposable
{
    ...
    [Fact]
    public void BankPath_ProjectScope_IsDataRootAiRacconMemoryDb()
    {
        Factory(InstallScope.Project).BankPath.ShouldBe(Path.Combine(_dataRoot, ".ai-raccon", "memory.db"));
    }

    [Fact]
    public void MetaDatabasePath_UserScope_IsDataRootRacconMetaDb()
    {
        Factory().MetaDatabasePath.ShouldBe(Path.Combine(_dataRoot, "raccon_meta.db"));
    }

    [Fact]
    public void MetaDatabasePath_ProjectScope_IsDataRootAiRacconRacconMetaDb()
    {
        Factory(InstallScope.Project).MetaDatabasePath.ShouldBe(Path.Combine(_dataRoot, ".ai-raccon", "raccon_meta.db"));
    }
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "airaccon-store-tests", Guid.NewGuid().ToString("N"));
