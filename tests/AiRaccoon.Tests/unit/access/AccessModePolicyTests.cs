...
namespace AiRaccoon.Tests.Access;
...
public sealed class AccessModePolicyTests
{
    ...
    [Fact]
    public void Resolve_NothingSet_DefaultsToRw()
    {
        AccessModePolicy.Resolve(null, null).ShouldBe(AccessMode.Rw);
    }
    ...
    public void Allows_Read_IsAllowedInEveryMode(AccessMode mode)
    {
        AccessModePolicy.Allows(mode, AccessRequirement.Read).ShouldBeTrue();
    }
    ...
    public void Allows_Write_RequiresRwOrFull(AccessMode mode, bool allowed)
    {
        AccessModePolicy.Allows(mode, AccessRequirement.Write).ShouldBe(allowed);
    }
    ...
    [InlineData(AccessMode.Full, true)]
    public void Allows_Destructive_RequiresFull(AccessMode mode, bool allowed)
    {
        AccessModePolicy.Allows(mode, AccessRequirement.Destructive).ShouldBe(allowed);
    }
    ...
    [InlineData(" full ", AccessMode.Full)]
    public void Parse_AcceptsModesCaseInsensitively(string value, AccessMode expected)
    {
        AccessModePolicy.Parse(value).ShouldBe(expected);
    }
    ...
    public void Parse_RejectsUnknownValues(string? value)
    {
        AccessModePolicy.Parse(value).ShouldBeNull();
    }
