...
public class CliCommandTreeTests
{
    ...
    public void Root_ExposesAllVerbFamilies()
    {
        ...
        root.Children.OfType<Command>().Select(c => c.Name)
            .ShouldBe(["access", "model", "retrieval", "sweep", "noise", "sync", "ingest", "watch", "encryption", "extract", "maintenance", "serve"]);
        CliCommandTree.Verbs.ShouldBe(["access", "model", "retrieval", "sweep", "noise", "sync", "ingest", "watch", "encryption", "extract", "maintenance", "serve"]);
    }
