...
public class SweepConfigKeysTests
{
    ...
    [InlineData("False")]
    [InlineData("FALSE")]
    public void ParseEnabled_OnlyFalseTurnsItOff(string value) =>
        SweepConfigKeys.ParseEnabled(value).ShouldBeFalse();
    ...
    public void ParseIntervalHours_ClampsToTheSupportedRange(string? value, int expected) =>
        SweepConfigKeys.ParseIntervalHours(value).ShouldBe(expected);
}
