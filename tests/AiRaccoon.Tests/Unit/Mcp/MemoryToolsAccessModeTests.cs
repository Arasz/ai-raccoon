...
public sealed class MemoryToolsAccessModeTests
{
    ...
    public MemoryToolsAccessModeTests()
    {
        ...
        _tools = new MemoryTools(_store, gate, new SearchDispatcher(_store, new NoOpCodeSearchService(), new NoOpSearchQualityService()), new QueryGuardService(_store), new MemoryWriteService(_store, new FakePromotionQueue()), new NoOpMeasurementRecorder(), _store, NullLogger<MemoryTools>.Instance, new CountingEmbeddingService());
