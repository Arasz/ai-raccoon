...
public class MemoryToolsTests
{
    ...
    private sealed class FakeStore : IMemoryStore
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return Task.FromResult(new AddContentResult(SharedEntry ?? new MemoryEntry(hash, "p.md", ContextNaming.SharedContext, "v", 1), true));
        }
        ...
        public Task<AddContentResult> AddContentAsync(string projectId, string path, string content, string? context,
            ...
            Task.FromResult(new AddContentResult(new MemoryEntry("new-hash", path, context ?? "project:acme", content, 1), true));
