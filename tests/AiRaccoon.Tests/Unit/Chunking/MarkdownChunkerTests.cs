...
public class MarkdownChunkerTests
{
    ...
    public void Split_IdenticalInput_ProducesIdenticalChunks()
    {
        ...
        var first = MarkdownChunker.Split(text, 10, 5, CharCount);
        var second = MarkdownChunker.Split(text, 10, 5, CharCount);
    ...
    {
        var chunks = MarkdownChunker.Split("", 10, 0, CharCount);
    ...
    public void Split_WithMaxTokensZero_Throws() =>
        Should.Throw<ArgumentOutOfRangeException>(() =>
            MarkdownChunker.Split("aa", 0, 0, CharCount));

    [Fact]
    public void Split_WithNegativeOverlay_Throws() =>
        Should.Throw<ArgumentOutOfRangeException>(() =>
            MarkdownChunker.Split("aa", 10, -1, CharCount));

    [Fact]
    public void Split_WithOverlayNotBelowMaxTokens_Throws() =>
        Should.Throw<ArgumentOutOfRangeException>(() =>
            MarkdownChunker.Split("aa", 10, 10, CharCount));

    [Fact]
    public void Split_WithNullText_Throws() =>
        Should.Throw<ArgumentNullException>(() =>
            MarkdownChunker.Split(null!, 10, 0, CharCount));

    [Fact]
    public void Split_WithNullTokenCounter_Throws() =>
        Should.Throw<ArgumentNullException>(() =>
            MarkdownChunker.Split("aa", 10, 0, null!));
}
