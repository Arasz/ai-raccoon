...
///     (docs/plans/2026-08-17-search-phase-attribution.md): <c>SearchAsync</c>'s nine phase timings
///     (open, embed, vector, fts, fusion, merge, adjustment, snippets, bump) and its measured
///     total, previously <c>SearchTimings.Empty</c>. <c>fts</c>/<c>vector</c>
...
public sealed class SqliteMemoryStoreSearchTimingsTests : IDisposable
{
    ...
    public async Task Search_PopulatesEveryPhaseThatRan_AndLeavesTheSkippedModalityAtZero()
    {
        ...
        // A matching entry so the search returns results: snippet resolution and the access bump
        // only measure work they actually do, so an empty bank would leave them at zero.
        ...
        result.Timings.Adjustment.ShouldBeGreaterThan(TimeSpan.Zero);
    ...
    public async Task Search_AttributesEachPhasesElapsedTime_ToThatPhaseAndNoOther()
    {
        ...
        // embed, vector, fts, fusion, merge, adjustment, snippets, bump — every gap between
        // brackets (and the gap before Total's own closing read) scripted at zero, so Total's
        // nesting around every phase does not perturb any individual phase's own value. A
        // single-token query scoped to the project has exactly one context and no FTS fallback
        // (FtsQueryNormalizer gives Fallback = null for a one-token query), so each phase brackets
        // exactly once. The bank is seeded so the search has a result: the deferred-snippet path
        // must run for the snippets phase to close its bracket (and the access bump to do work).
        var timeProvider = ScriptedTimeProvider.ForPhases(
            ...
            TimeSpan.FromMilliseconds(33), // fts
            TimeSpan.FromMilliseconds(44), // fusion
            TimeSpan.FromMilliseconds(55), // merge
            TimeSpan.FromMilliseconds(66), // adjustment
            TimeSpan.FromMilliseconds(77), // snippets
            TimeSpan.FromMilliseconds(88) // bump
        );
        var store = SearchTimingsHarness.CreateStore(_factory, timeProvider, new SearchTimingsHarness.VectorEmbedderStub());
        var ct = TestContext.Current.CancellationToken;

        await store.WriteAsync(new MemoryWriteRequest("proj-1", "a widget for sale"), ct);
        ...
        result.Timings.Fts.ShouldBe(TimeSpan.FromMilliseconds(33));
        result.Timings.Fusion.ShouldBe(TimeSpan.FromMilliseconds(44));
        result.Timings.Merge.ShouldBe(TimeSpan.FromMilliseconds(55));
        result.Timings.Adjustment.ShouldBe(TimeSpan.FromMilliseconds(66));
        result.Timings.Snippets.ShouldBe(TimeSpan.FromMilliseconds(77));
        result.Timings.Bump.ShouldBe(TimeSpan.FromMilliseconds(88));
    }
    ...
    private sealed class ScriptedTimeProvider(IReadOnlyList<TimeSpan> deltas) : TimeProvider
    {
        ...
        ///     of the nine phases in measurement order, plus a trailing zero gap before Total's
        ///     own closing read.
        /// </summary>
