...
public sealed class RetrievalBaselineTests : IDisposable
{
    ...
    public RetrievalBaselineTests()
    {
        ...
        _store = new SqliteMemoryStore(factory, new FakeTimeProvider(FixedNow),
            new AiRaccoon.Infrastructure.Chunking.TokenizerChunker(),
    ...
    public async Task RunAllBaselineQueries_ReportsMatchStatistics()
    {
        ...
        foreach (var query in queries)
        {
            ...
            var mappedResults = results.Select((r, i) => new ResultEntry(
                Rank: i + 1, Hash: r.Hash, Ranking: r.Ranking,
                Path: r.Path, Snippet: r.Snippet,
                IsExpectedSource: query.ExpectedSource is not null
    ...
    private static string ClassifyFile(string relPath)
    {
        ...
        if (relPath is "docs/architecture.md" or "docs/data-model.md"
            or "docs/flows.md" or "docs/requirements.md" or "docs/CHANGELOG.md") { return "architecture"; }
        if (relPath.StartsWith("docs/explanation/")) { return "explanation"; }
        if (relPath.StartsWith("docs/how-to/")) { return "howto"; }
        if (relPath.StartsWith("docs/reference/")) { return "reference"; }
        if (relPath.StartsWith("docs/rules/")) { return "rules"; }
        if (relPath.StartsWith(".ai-badger/invariants/")) { return "invariants"; }
        if (relPath.Contains("/skills/") && relPath.EndsWith("SKILL.md")) { return "skills"; }
        if (relPath.StartsWith(".ai-badger/agents/")) { return "agents"; }
        if (relPath.StartsWith(".ai-badger/instructions/")) { return "instructions"; }
        if (relPath.StartsWith(".ai-badger/") && relPath.EndsWith(".json")) { return "config"; }
        if (relPath.StartsWith(".ai-badger/") && relPath.EndsWith(".md")) { return "config"; }
        if (relPath.StartsWith(".ai-badger/agent-instructions/")) { return "agent-model"; }
        if (relPath.StartsWith(".remember/")) { return "remember"; }
        if (relPath.StartsWith("docs/tutorials/")) { return "tutorials"; }
        if (relPath.StartsWith("docs/legacy/")) { return "legacy"; }
        if (relPath.StartsWith("docs/meta/")) { return "meta"; }
        if (relPath is "README.md" or "CLAUDE.md" or "HERMES.md" or "REVIEW.md") { return "root-md"; }
        if (relPath.StartsWith("infra/")) { return "infra"; }
        return "unknown";
    }
    ...
    private static string SourcePrefix(string relPath, string fileType)
    {
        ...
        if (relPath.StartsWith(".ai-badger/")) { return "ai-badger"; }
        if (relPath.StartsWith(".remember/")) { return "remember"; }
    ...
    private static string ShortRel(string relPath)
    {
        ...
        if (relPath.StartsWith(".ai-badger/")) { return relPath[".ai-badger/".Length..]; }
        if (relPath.StartsWith(".remember/")) { return relPath[".remember/".Length..]; }
        return relPath;
    }
    ...
    private static string SanitizeSection(string heading)
    {
        return heading.Replace(' ', '-').Replace("/", "-").ToLowerInvariant();
    }
    ...
    public sealed record BaselineQuery(
        ...
        string? ExpectedSource, string? ExpectedKnowledge,
        string Scope, int SearchLimit, bool NegativeTest);
    ...
    public sealed record ResultEntry(
        int Rank, string Hash, double Ranking, string Path, string Snippet, bool IsExpectedSource);

    public sealed record BaselineReport(
        string ProjectId, DateTimeOffset ExportedAt,
        int QueryCount, int QueriesWithResults, int ExpectedSourceMatchesAtTop3,
