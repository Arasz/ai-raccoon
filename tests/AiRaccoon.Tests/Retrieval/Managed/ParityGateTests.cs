...
using AiRaccoon.Tests.Retrieval.Metrics;
using AiRaccoon.Tests.Retrieval.Reference;
using AiRaccoon.Tests.Retrieval.Sweep;
using Shouldly;
using Xunit;

namespace AiRaccoon.Tests.Retrieval.Managed;
...
public sealed class ParityGateTests
{
    ...
    private readonly ManagedHarnessFixture _fixture;
    private readonly ITestOutputHelper _output;

    public ParityGateTests(ManagedHarnessFixture fixture, ITestOutputHelper output)
    {
        _fixture = fixture;
        _output = output;
    }
    ...
    {
        _fixture.Harness.DocumentCount.ShouldBe(RealWorldCorpus.Documents.Count,
        ...
        var outcomes = await SweepRunner.RunAsync(
            RealWorldQueries.Queries, _fixture.Harness.RankAsync, TestContext.Current.CancellationToken);
        ...
        var p95 = Percentile(_fixture.Harness.QueryLatenciesMs, 0.95);
        ...
        _output.WriteLine($"reference (golden k={golden.K}): nDCG@10 {reference.NdcgAt10:F4}, MRR {reference.Mrr:F4}, " +
                          $"Recall@10 {reference.RecallAt10:F4}");
        _output.WriteLine($"new-side p95 {p95:F1} ms / p50 {Percentile(_fixture.Harness.QueryLatenciesMs, 0.50):F1} ms " +
                          $"over {_fixture.Harness.QueryLatenciesMs.Count} queries");
        _output.WriteLine(rows.ToString());
    ...
    public async Task DegenerateQuerySubset_NoRegression_AtDefaultFusionConfig()
    {
        ...
        {
            var ranked = await _fixture.Harness.RankAsync(ManagedHarness.DefaultPoint, query,
        ...
        _output.WriteLine($"degenerate subset ({degenerate.Count} queries): new-side nDCG@10 {subsetNdcg.Average():F4} " +
        ...
        IReadOnlyList<SweepOutcome> outcomes, double p95)
    {
        ...
        report.AppendLine($"|---|---|---|---|");
        report.AppendLine($"| {Percentile(_fixture.Harness.QueryLatenciesMs, 0.50):F1} ms | {p95:F1} ms | " +
                          $"{( _fixture.Harness.QueryLatenciesMs.Count == 0 ? 0 : _fixture.Harness.QueryLatenciesMs.Max()):F1} ms | " +
                          $"{_fixture.Harness.QueryLatenciesMs.Count} |");

        File.WriteAllText(Path.Combine(managedDir, "parity-report.md"), report.ToString());
        _output.WriteLine($"wrote {Path.Combine(managedDir, "parity-report.md")}");
    }
    ...
    private AuditResult PerQueryAudit(GoldenFile golden)
    {
        ...
        foreach (var query in RealWorldQueries.Queries)
        {
            ...
            var newRanked = _fixture.Harness.RankAsync(ManagedHarness.DefaultPoint, query,
    ...
    private sealed record AuditResult(int Better, int Within, int Worse, double WorstRegression, string WorstQueryId,
