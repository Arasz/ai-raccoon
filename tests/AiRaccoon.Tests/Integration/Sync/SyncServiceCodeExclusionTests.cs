...
public sealed class SyncServiceCodeExclusionTests : IDisposable
{
    ...
    // --- P2: telemetry (search_quality/metrics) never syncs (ADR-0098) -------------------------
    ...
    {
        names.ShouldNotContain("search_quality", "telemetry must never leave the machine via sync (ADR-0098).");
