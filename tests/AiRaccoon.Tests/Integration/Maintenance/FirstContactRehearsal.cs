...
public sealed class FirstContactRehearsal(ITestOutputHelper output)
{
    ...
    public async Task Rehearse_TheFirstOpenByANewBinary()
    {
        ...
        var outcomes = await new MaintenanceJobRunner(TimeProvider.System, new NoOpMeasurementRecorder(),
            ...
            [
                new ChunkBackfillJob(TestData.RealFileTypeMatcher(), TestData.RealMarkdownChunker(),
                    TestData.RealPlainTextChunker(), TimeProvider.System, TestData.CreateEmbeddingService()),
                new Vec0ReclaimJob(),
                new VacuumJob()
                        ...
                        && int.TryParse(rawSample, out var sample) && sample > 0)
        {
            ...
            var embedder = TestData.CreateEntryEmbedder(TestData.CreateEmbeddingService(), _modelMigrationLease, _timeProvider, new VecDimensionReconciler());
