...
## What's new
- **1.33.0 makes code ingestion cheaper on the machine: all embedding runs through one bounded, coalescing drain (`EventPump`, ADR-0091) instead of three uncoordinated embedders, the ONNX session's intra-op threads are capped at half the cores (`settings model threads <n>`), and each drain pass is bounded by `settings maintenance embed-rows-per-run <n>` (default 128, ceiling 4096); the re-ingest prune reaches `code_entries`, a watched code file signals the code corpus, `FileIngestor` takes required dependencies, `doctor` and the log show the thread count that took effect, and `search.adjustment` is exported.** (1.33.0) [#507](https://github.com/Arasz/ai-raccoon/pull/507) · [#517](https://github.com/Arasz/ai-raccoon/pull/517) · [#518](https://github.com/Arasz/ai-raccoon/pull/518) · [#492](https://github.com/Arasz/ai-raccoon/pull/492) · [#490](https://github.com/Arasz/ai-raccoon/pull/490) · [#481](https://github.com/Arasz/ai-raccoon/pull/481) · [#483](https://github.com/Arasz/ai-raccoon/pull/483) · [#524](https://github.com/Arasz/ai-raccoon/pull/524)
- **1.32.3 fixes a watch data-loss bug: a newly created directory's own filesystem event was digested as a deleted file and cascade-deleted the files inside it, racing their own digests — `mkdir` + write inside a watched repo could silently drop the new files from memory.** (1.32.3) [#509](https://github.com/Arasz/ai-raccoon/issues/509)
- **1.32.2 closes two defects the 1.32.1 check found in the watch and the chunker: filesystem events under hidden or deny-set directories (agent worktrees, `node_modules`, `bin`) are no longer digested by a repo-root watch and the next scan prunes what already leaked; a heading line opens exactly one chunk, so a chunk is never labelled with the next section.** (1.32.2) [#494](https://github.com/Arasz/ai-raccoon/issues/494) · [#489](https://github.com/Arasz/ai-raccoon/issues/489)
