...
- **Two tables that grew forever now have a reaper (1.14.0).** `promotion_discards` (965 rows on one live bank, more than everything else the promotion feature had produced) and `search_quality` (one row per search, forever) had no `DELETE` anywhere in `src/`, while `noise_entries` had a working reaper the pattern was never generalised from. Both are now age-bounded in the maintenance pass behind settings — `maintenance.promotion-discard-retention-days.global` (180) and `maintenance.search-quality-retention-days.global` (90). The discard purge requires age **and** an absent entry: while the entry lives, its discard is the only thing stopping the propose path re-offering a candidate you already rejected, so age alone would re-inflict the rejection. [ADR-0055](docs/adr/0055-a-discard-is-load-bearing-while-its-entry-lives.md)
- **A search hit's `rating` no longer drifts below its own access history (1.14.0).** `BumpAccessAsync` incremented `access_count` with a relative expression — safe under interleaving — and wrote `rating` as a literal computed from a value read in an earlier round trip, which is not. Two concurrent hits on one hash both read 5, both wrote the rating of 6, and the row ended at count 7 with the rating of 6. `rating` is the sweep's deletion input, so an entry could be swept on a score lower than it had earned. Now computed in the same statement that increments the count — atomic by construction, and one round trip cheaper. [ADR-0053](docs/adr/0053-rating-is-computed-where-it-is-stored.md)
- **The published retrieval numbers are in-sample, and now say so (1.14.0).** The headline metrics gate asserted only that nDCG/MRR/recall lay in `[0, 1]` — values those functions return for any input, including a reversed ranking — and the same 11 queries both selected the fusion parameters and gated them. Measured on one path against three documents no sweep had seen: **in-sample 0.673, held out 0.285.** Out-of-sample retrieval is 42% of every figure ADR-0005 and ADR-0006 publish, and both now carry that label. This changes no behaviour; it changes what the numbers mean. [ADR-0056](docs/adr/0056-a-retrieval-gate-measured-off-its-tuning-set.md)
- **Workspaces no longer require `full` (1.13.0).** `memory_workspace_consolidate` and `memory_workspace_discard` were gated as destructive operations, so at the default `rw` an agent could *open* a sandbox and then neither finish nor abandon it — which pushed installs to `access.mode.global = full` just to use workspaces, manufacturing the precondition for the cross-project delete above. Neither reaches committed memory: consolidate promotes into the caller's own project, and discard's filter binds both `workspace_id` and `project_id`. Both are now `rw`; `ro` still refuses them. **If you are moving to `rw` because of this, install 1.13.0 first** — flipping the mode on an older build breaks workspaces the moment an agent tries to close one. Automatic sweeping is unaffected either way; the background sweep never goes through the tool gate. [ADR-0052](docs/adr/0052-the-workspace-lifecycle-is-a-write-not-a-destruction.md)
