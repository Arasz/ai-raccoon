...
- Keep every agent-facing instruction file (CLAUDE.md-equivalent, Copilot/other agent instructions, scoped path instructions) consistent when changing shared agent policy — use a single machine-readable model as the source of truth if one
  exists in this project, rather than hand-editing each file independently.
