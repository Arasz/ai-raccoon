...
- Run the project's configured lint command (`commands.lint` in `.ai-badger/config.json`, listed under Commands in this repo's agent instructions) in CI. If the project has not chosen a tool yet, prefer one that lints and formats (ruff); if it has, use that one and do not argue style in review.
