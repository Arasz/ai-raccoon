"""DotnetToolSettings.xml rewrite for a PackAsTool shell nupkg.

The SDK emits exactly one RuntimeIdentifierPackage (the RID it was packed for); a
multi-RID tool shell must list all of them or `dotnet tool install` rejects every
other platform. Self-gating: every function exits nonzero if the input shape is
unexpected or the patched package does not contain every requested RID.
"""
...
def parse_settings(content: str) -> Tuple[str, str, Optional[str], Optional[str]]:
    """Returns (tool command name, id prefix, entry point, runner). Entry point and
    runner are optional — absent in the packed shell shape, present in the
    intermediate SDK shape — and preserved when seen."""
    command = None
    prefix = None
    entry_point = None
    runner = None
...
def patch(nupkg: Path, rids: List[str]) -> Tuple[str, str]:
    """Rewrites the nupkg's DotnetToolSettings.xml to list every rid; verifies the
    rewrite. Returns (command, prefix). Raises SystemExit on any unexpected shape."""
