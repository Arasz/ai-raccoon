"""A process's resident set and physical footprint, current and peak, in KiB (Linux and macOS).
...
def memory_kib(pid: int | None = None) -> MemorySample:
    """This process's memory, or another process's by pid (macOS: one this user may inspect)."""
    target = os.getpid() if pid is None else pid
    if sys.platform == "darwin":
        libproc = ctypes.CDLL("/usr/lib/libproc.dylib", use_errno=True)
        # ru_maxrss (bytes on macOS) only exists for this process; another pid's peak RSS is its current RSS.
        peak_rss = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss // 1024 if pid is None else 0

        info6 = _RusageInfoV6()
        if libproc.proc_pid_rusage(target, _RUSAGE_INFO_V6, ctypes.byref(info6)) == 0:
            return _sample_from_v6(info6, peak_rss)

        info4 = _RusageInfoV4()
        if libproc.proc_pid_rusage(target, _RUSAGE_INFO_V4, ctypes.byref(info4)) != 0:
    ...
    status_path = Path("/proc/self/status" if pid is None else f"/proc/{pid}/status")
