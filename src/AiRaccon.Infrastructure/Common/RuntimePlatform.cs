namespace AiRaccon.Infrastructure.Common;
...
internal static class RuntimePlatform
{
    ...
    public static bool TryGetSupportedAssetPlatform(string rid, out string platform)
        => SupportedAssetPlatforms.TryGetValue(rid, out platform!);

    /// <summary>Asset platform for error reporting, including RIDs upstream has no binaries for.</summary>
    public static string ExpectedAssetPlatform(string rid) => rid switch
    {
    ...
    public static string ModuleExtension(string rid) => rid switch
    {
