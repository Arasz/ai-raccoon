...
namespace AiRaccon.Core.Rating;
...
/// </summary>
public sealed class MemoryExtensionHost : IMemoryStore
{
    private readonly IMemoryStore _inner;
    private readonly IReadOnlyList<IMemoryExtension> _extensions;

    public MemoryExtensionHost(IMemoryStore inner, IEnumerable<IMemoryExtension> extensions)
    {
    ...
    public async Task<MemoryEntry> WriteAsync(MemoryWriteRequest request, CancellationToken cancellationToken = default)
    {
        foreach (var extension in _extensions)
        {
            await extension.OnWriteAsync(
                new WriteContext(request.ProjectId, request.Content, request.Context, request.AgentId, request.WorkspaceId),
    ...
    public async Task<IReadOnlyList<MemorySearchResult>> SearchAsync(SearchQuery query, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var extension in _extensions)
        {
            await extension.OnSearchAsync(
                new SearchContext(query.ProjectId, query.Query, results, query.WorkspaceId, query.Limit, query.MinScore),
                cancellationToken).ConfigureAwait(false);
        }
    ...
    public async Task<int> DeleteContextAsync(string projectId, string context, CancellationToken cancellationToken = default)
    {
        foreach (var extension in _extensions)
        {
            await extension.OnDeleteAsync(new DeleteContext(projectId, Context: context), cancellationToken).ConfigureAwait(false);
        }
    ...
    public async Task<EmbeddingConfig> ConfigureEmbeddingAsync(string projectId, string provider, string model, string? apiKey, CancellationToken cancellationToken = default)
        => await _inner.ConfigureEmbeddingAsync(projectId, provider, model, apiKey, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<MemoryEntry> AddContentAsync(string projectId, string path, string content, string? context, CancellationToken cancellationToken = default)
