...
public sealed class MemoryPrompts
{
        ...
        string? projectId = null)
    {
        ...
        return $"""
                ...
                - Promote durable, cross-project knowledge with memory_share — shared entries are curated and never swept. Nothing is shared without an explicit promotion. The shared tier holds the FIRST chunk promoted per source file (queue order) — later chunks of the same file are reported `absorbed`, not promoted, and the queue drops them; re-propose can recover them from the project rows.
