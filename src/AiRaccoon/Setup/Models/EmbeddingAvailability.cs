...
public sealed partial class EmbeddingAvailability(ILogger<EmbeddingAvailability> logger, IBundledModel bundledModel) : IEmbeddingAvailability
{
    ...
    {
        [LoggerMessage(EventId = 40, Level = LogLevel.Warning, Message = "Bundled embedding model unavailable ({Errors}); run 'ai-raccoon model embedding set local' to restore it")]
        public static partial void BundledEmbedingModuleUnavailable(ILogger logger, string errors);

        [LoggerMessage(EventId = 41, Level = LogLevel.Error, Message = "Bundled embedding model check failed; run 'ai-raccoon model embedding set local' to restore it")]
