...
///     A null <see cref="Value" /> is "not configured"; a null state (the reader's null return) is
///     "unreadable" — the distinction P1 §1.3 requires so a failed read never prints a false remedy.
/// </summary>
internal sealed record CorpusEngineState(string? Value, string? Detail, string? Suffix, long? PendingRows);
