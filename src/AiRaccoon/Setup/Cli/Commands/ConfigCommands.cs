        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            return commandPath switch
            {
                ...
                ["sweep", "enable"] => await (settings ?? ThrowHelper.ThrowArgumentNullException<SettingsCommands>(nameof(settings))).SweepEnabledSetAsync(true, store, stdout, cancellationToken),
                ["sweep", "disable"] => await (settings ?? ThrowHelper.ThrowArgumentNullException<SettingsCommands>(nameof(settings))).SweepEnabledSetAsync(false, store, stdout, cancellationToken),
                ["sweep", "interval-hours"] => await (settings ?? ThrowHelper.ThrowArgumentNullException<SettingsCommands>(nameof(settings))).SweepIntervalHoursSetAsync(parseResult, store, stdout, stderr, cancellationToken),
                ...
                ["ingest", "scope", "add"] or ["watch", "scope", "add"] => await (watch ?? ThrowHelper.ThrowArgumentNullException<WatchCommands>(nameof(watch))).ScopeAddAsync(parseResult, store, stdout, cancellationToken),
                ["ingest", "scope", "remove"] or ["watch", "scope", "remove"] => await (watch ?? ThrowHelper.ThrowArgumentNullException<WatchCommands>(nameof(watch))).ScopeRemoveAsync(parseResult, store, stdout, cancellationToken),
                ["ingest", "scope", "list"] or ["watch", "scope", "list"] => await (watch ?? ThrowHelper.ThrowArgumentNullException<WatchCommands>(nameof(watch))).ScopeListAsync(parseResult, store, stdout, cancellationToken),
