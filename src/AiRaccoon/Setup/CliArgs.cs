...
internal static class CliArgs
{
    ...
    private static RootCommand BuildFullRootCommand()
    {
    ...
    private static Command SyncCommand()
    {
        ...
        var s3 = new Command("s3", "S3-compatible endpoint (credentials are persisted in the settings table)")
        {
            ...
            new Option<string>("--access-key") { Description = "S3 access key persisted in the settings table", HelpName = "key" },
            new Option<string>("--secret-key") { Description = "S3 secret key persisted in the settings table", HelpName = "key" }
        };
        add.Add(s3);
        sync.Add(add);
