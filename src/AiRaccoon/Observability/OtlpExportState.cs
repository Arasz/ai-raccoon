...
public sealed record OtlpExportState(bool Enabled, string? Endpoint, string? Protocol, string? InvalidEndpointReason = null)
{
    ...
    /// and a malformed one are both "disabled", but only the latter carries a reason.</summary>
    ...
    /// <summary>UriKind.Absolute alone still accepts "localhost:4318" (Scheme="localhost", A5), so
    /// the scheme is checked explicitly too.</summary>
