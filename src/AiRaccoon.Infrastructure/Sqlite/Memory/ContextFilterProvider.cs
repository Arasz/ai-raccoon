...
/// </summary>
internal static class ContextFilterProvider
{
    /// <summary>
    ...
    ///     authorize.
    /// </summary>
    public static ContextFilter For(string context, string projectId, string alias)
    {
        if (context == ContextNaming.SharedContext)
        {
            return new ContextFilter($"{alias}scope = 'shared'", []);
        }
        ...
        {
            return new ContextFilter($"{alias}workspace_id = @workspaceId AND {alias}project_id = @projectId",
        ...
        return new ContextFilter($"{alias}scope = 'custom' AND {alias}context_label = @contextLabel AND {alias}project_id = @projectId",
            new Dictionary<string, object?> { ["contextLabel"] = context, ["projectId"] = projectId });
    }
}

public sealed record ContextFilter(string Filter, Dictionary<string, object?> Values);
