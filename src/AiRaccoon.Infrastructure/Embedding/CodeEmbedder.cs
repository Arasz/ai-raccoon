    ...
    : ICodeEmbedder
{
    ...
    {
        var codeModel = await ReadCodeModelAsync(connection, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var engine = await ReadCodeEngineAsync(connection, cancellationToken);

        var rows = (await connection.QueryAsync<EmbedRow>(new CommandDefinition(
                MemorySql.SelectAllPendingCodeForEmbed, new { limit }, cancellationToken: cancellationToken))).ToList();
        if (rows.Count == 0)
        {
            return 0;
        }
    ...
    public async Task<bool> ReconcileFingerprintAsync(SqliteConnection connection, CancellationToken cancellationToken)
    {
        ...
        var storedFingerprint = await ReadCodeEngineAsync(connection, cancellationToken);
        ...
        try
        {
            ...
            await vecDimensions.ReconcileCodeAsync(connection, transaction, dimensions, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await vecDimensions.ReconcileCodeAsync(connection, target, cancellationToken);
    }
