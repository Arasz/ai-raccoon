...
public sealed class MemoryTools
{
    ...
    [Description("Configures the bank's embedding model: provider 'local' with a GGUF model path, or a remote provider name with a model id. Remote requires an API key (env AIRACCON_VECTORSSPACE_API_KEY or api_key).")]
    ...
    [Description("Syncs the bank's committed contexts (shared + project:<id>) to the configured cloud database. Requires AIRACCON_SQLITECLOUD_DB_ID and AIRACCON_SQLITECLOUD_API_KEY.")]
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new McpException("sync-not-configured: set AIRACCON_SQLITECLOUD_DB_ID and AIRACCON_SQLITECLOUD_API_KEY");
        }
    }
